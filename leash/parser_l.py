...
class Parser:
    ...
    def eat(self, token_type):
        if self.current().type == token_type:
            ...
            raise LeashError(f"Expected {token_type}, but found {tok.type} ('{tok.value}')", tok.line, tok.column, tip=tip)
    ...
    def parse_function(self):
        ...
        if self.current().type != 'COLON':
            tok = self.current()
            raise LeashError(f"Expected a colon (':') before the return type, but found {tok.type} ('{tok.value}')", 
                             tok.line, tok.column, 
    ...
    def parse_statement(self):
        ...
        if current.type == 'IF':
            ...
            if self.current().type == 'ELSE' and self.peek() and self.peek().type == 'IF':
                # Catch else if for helpful tip
                tok = self.current()
                raise LeashError("Found 'else if' instead of 'also'", tok.line, tok.column,
            ...
            cond = self.parse_expression(no_struct_init=True)
            self.eat('SEMI')
            step_target = self.parse_primary() # For simplicity let's assume primary is LValue
            ...
            raise LeashError(f"Unexpected token {tok.type} ('{tok.value}') at start of statement", tok.line, tok.column, tip=tip)
    ...
    def parse_primary(self, no_struct_init=False):
        if self.current().type == 'NUMBER':
            ...
            tok = self.current()
            raise LeashError(f"Unexpected token {tok.type} ('{tok.value}') in expression", tok.line, tok.column)
