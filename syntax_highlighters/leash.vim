...
" Keywords
 syn keyword leashKeyword fnc def struct union enum class type template macro opdef return if also else unless while for do foreach loop in imut vector vec matrix this thisop pub priv static stop continue empty ignore use works otherwise switch case default unsafe as inline defer error throw self alsou create del is isnt worker spawn shared fusion thisworker
